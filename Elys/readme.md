...
```
mkdir -p ~/.elys/cosmovisor/upgrades/v0.51.0/bin
git clone https://github.com/elys-network/elys.git
cd elys
git fetch
git checkout v0.51.0
make install
cp -a ~/go/bin/elysd ~/.elys/cosmovisor/upgrades/v0.51.0/bin/elysd
```
```
...
sed -i.bak -e "s%^address = \"tcp://localhost:1317\"%address = \"tcp://localhost:16317\"%; s%^address = \"localhost:9090\"%address = \"localhost:16090\"%" $HOME/.xiond/config/app.toml
```
